...
def run_one_time_project(lock):
    lock.acquire()
    global project_list
    global name_list
    while(1):
        try:
            f = open(next(project_list), "r")
            name= next(name_list)
        except:
...
if(__name__=="__main__"):
    ...
    th1 = multiprocessing.Process(target=run_one_time_project,args=(LOCK,))
    th2 = multiprocessing.Process(target=run_one_time_project,args=(LOCK,))
    th3 = multiprocessing.Process(target=run_one_time_project,args=(LOCK,))
    th4 = multiprocessing.Process(target=run_one_time_project,args=(LOCK,))
    # 启动线程
    th1.start()
    th2.start()
    th3.start()
