...
'''
连续化,区域化处理整个空间网络的参数拟合   ////Windows版本   Linux与Windows上面Python3关于全局变量的变化域似乎有些不同，故此处做一修改
...
for x in range(29):
    for y in range(30):
        if ((not os.path.exists("../data/net_saved_30_40_30_6_5_new/net_" + str(x) + "_" + str(y) + ".pkl"))):
            LIST_NUM.append([x, y])
...
def run_one_time_project(x_y):
    try:
        ...
        name = "../data/net_saved_30_40_30_6_5_new/net_" + str(x_y[0]) + "_" + str(x_y[1]) + ".pkl"
        #训练开启代码
        train(name, f)
    except:
