...
所获得的csv文件 共有368行，分别是每个区域（共87*75个区域）
中训练的数据 

每行中前30个是当前污染物削减系数 后4个为2*2区间的数据值
'''
...
def get_i_j_file():
    #以2*2为小区域进行神经网络输出值进行拟合，适当考虑模型的区域相关关系
    for i in range(87):
        for j in range(75):
            ...
            case_item = []  #获取368*4训练数据
            for CMAQ_RESULT in CMAQ_RESULT_LIST:
                net_item= []  #获取4个小区域训练数据
                for m in range(x_start,x_start+2):
                    for n in range(y_start,y_start+2):
                        try:
            ...
            try:
                fout=open("./data/output_2_2_new/out_"+str(i)+'_'+str(j)+".csv","w")
