...
每行中前30个是当前污染物削减系数 后region_def[0]*region_def[1]个为区域的数据值
'''
...
class Data_Process:
    ...
    def __init__(self,region_def):
        ...
        if(not os.path.exists("./data/output_"+str(self.REGION_DEF[0])+"_"+str(self.REGION_DEF[1])+"_new")):
            os.mkdir("./data/output_"+str(self.REGION_DEF[0])+"_"+str(self.REGION_DEF[1])+"_new")
...
if(__name__=='__main__'):
    data_process_object = Data_Process([6,10])
