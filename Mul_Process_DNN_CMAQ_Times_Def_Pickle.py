...
class CMAQ_PREDICT(object):
    ...
    def __init__(self, cuda_available, region_def, run_times, process_num, net_structure):
        ...
        #自动生成网络结果存储目录
        path = "./data/All_Net_Picklized/"+str(math.ceil(time.time()))
    ...
    def run(self):
        ...
        res_final = pool.map(self.run_one_time_project, self.LIST_NUM)   #获得结果顺序不定，最终需要排序不过顺序肯定比IO速度快多了，pickle之后的net能否使用还需验证（讲道理的话是可以用的）
        # 进程池不再添加新的进程
        pool.close()
        # 主线程阻塞等待子线程结束
        pool.join()
...
if(__name__=="__main__"):
    # 构造函数，系数和训练参数初始化，实例化RSM_PREDICT类需要若干参数，分别为CUDA加速标志，计算区域定义（list），计算迭代次数，多进程并行计算核数，网络结构定义（list）
    predict_object = CMAQ_PREDICT(True,[6,5],100,2,[30,40,40,30])
    predict_object.run()