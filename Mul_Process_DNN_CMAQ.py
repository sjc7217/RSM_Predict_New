...
class CMAQ_PREDICT:
    ...
    # 构造函数，系数和训练参数初始化，实例化RSM_PREDICT类需要若干参数，分别为CUDA加速标志，计算区域定义（list），网络计算精度（出口），多进程并行计算核数，网络结构定义（list）
    def __init__(self,cuda_available,region_def,accuracy,process_num,net_structure):
        #CUDA加速标志
